...
name, authentication_status, username = authenticator.login("Login to Radiant Launch", "main")
...
if authentication_status == False:
    ...
    template = config['custom_templates'].get(mode.lower().replace(" (now)", "").replace(" (future)", ""), {})
    ...
    if st.sidebar.button("Launch with Grok!"):
        if not brief:
            ...
            def sync_crm(action: str, data: dict) -> str:
                if config['enable_mcp'] and 'mcp_url' in locals():
            ...
            crm_tool = Tool(name="CRMSync", description="Pull/push leads. Input: 'action:pull|push, data:dict_str'", func=lambda x: sync_crm(x.split(',')[0], eval(x.split(',')[1] if len(x.split(',')) > 1 else '{}')))
            ...
            content_tool = Tool(name="ContentGen", description="Gens HTML with Grok.", func=lambda x: generate_content(eval(x)))
...
# LandingPageAgent (dental-focused template)
